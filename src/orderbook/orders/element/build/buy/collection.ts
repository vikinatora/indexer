...
import { BaseBuilder } from "@reservoir0x/sdk/dist/element/builders/base";
...
import * as utils from "@/orderbook/orders/element/build/utils";
...
export const build = async (options: BuildOrderOptions) => {
  try {
    ...
    if (!options.excludeFlaggedTokens) {
      ...
      if (buildInfo.kind === "erc721") {
        builder = collectionResult.token_set_id.startsWith("contract:")
          ? new Sdk.Element.Builders.ContractWide(config.chainId)
          : new Sdk.Element.Builders.TokenRange(config.chainId);
      ...
      if (bitVectorCost == -1 || bitVectorCost > packedListCost) {
        ...
        if (buildInfo.kind === "erc721") {
          builder = new Sdk.Element.Builders.TokenList.PackedList(config.chainId);
        } else if (buildInfo.kind === "erc1155") {
          builder = new Sdk.Element.Builders.TokenList.PackedList(config.chainId);
        }
      } else {
        ...
        if (buildInfo.kind === "erc721") {
          builder = new Sdk.Element.Builders.TokenList.BitVector(config.chainId);
        } else if (buildInfo.kind === "erc1155") {
          builder = new Sdk.Element.Builders.TokenList.BitVector(config.chainId);
        }
      }
    ...
    logger.error("element-build-buy-collection-order", `Failed to build order: ${error}`);
    return undefined;
  }
};
